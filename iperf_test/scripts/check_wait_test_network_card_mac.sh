...
timeout 9 mysql -h192.168.0.10 -P3306 -uroot -p1qaz2wsx network_card_test -N --batch -e "`cat /workspace/tmp_data/update.sql`" 2>&1 | grep ERROR -c | awk -v mip=$manager_ip '{ if($1>0) {print "Fatal Error: Tester("$manager_ip") External Network Card Mac Address Update DB Failed!"} else {print "Update DB OK!"} }'
