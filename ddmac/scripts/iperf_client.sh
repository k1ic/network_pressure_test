...
#执行 iperf client 命令
awk -F ',' '{ print "ifconfig "$1" | grep inet"}' /workspace/tmp_data/enp-nic-id_mac.csv | sh | awk '{ print $2}' | awk -v t_time=$transmit_time -F '.' '{ip_s=$1"."$2"."$3"."$4-1; ip_c=$0; print "nohup iperf -c "ip_s" -p "10000+NR" -B "ip_c" -f m -i 1 -t "t_time" 2>&1 | grep -E '\''connected|0.0-"t_time".0'\'' | tr '\''\\n'\'' '\'' '\'' > /workspace/tmp_data/"10000+NR".log 2>&1 &"}' | sh
sleep $sleep_time
